...
#===xzy added for visualization GradCAM 
...
class Trainer:
    ...
    def select(self, train_loader, print_int=10, logger=None):
        ...
        device = self.device
        embedding_dim = self.model_old.module.head_channels
        new_classes_id = [x + self.old_classes for x in range(self.nb_new_classes)]
        bucket = torch.zeros(self.nb_new_classes, self.old_classes, embedding_dim, dtype=torch.float32).to(device)
        nums = torch.zeros(self.nb_new_classes, dtype = torch.long).to(device)
        ...
        for cur_step, (images, labels) in enumerate(train_loader):
            ...
            with torch.no_grad():
                ...
                outputs_old = torch.softmax(outputs_old, dim = 1).permute(0, 2, 3, 1).reshape(-1, self.old_classes)
                ...
                imprinting_w = torch.cat([x.weight for x in self.model_old.module.cls], dim=0).squeeze(-1).squeeze(-1)
                ...
                #===
                    ...
                    if cur_class_score.shape[0] != 0:
                        bucket[new_class_id - self.old_classes] = bucket[new_class_id - self.old_classes] + torch.sum(cur_class_score, dim = 0)
    ...
    def warm_up(self, cur_epoch, optim, train_loader, scheduler=None, print_int=10, logger=None):
        ...
        ukd_loss = torch.tensor(0.)
        if self.opts.unkd_in_warm:
            warm_unkd = UnbiasedKnowledgeDistillationLoss(reduction="none", alpha=self.opts.alpha)
        if hasattr(model.module, 'weight_old'):
            model.module.weight_old.requires_grad = True
        if hasattr(model.module, 'weight_new'):
            model.module.weight_new.requires_grad = True 
        if hasattr(model.module, 'new_bias'):
            model.module.new_bias.requires_grad = True
        if hasattr(model.module, 'weight_old_bg'):
            model.module.weight_old_bg.requires_grad = True
        if hasattr(model.module, 'weight_new_bg'):
            model.module.weight_new_bg.requires_grad = True
        if hasattr(model.module, 'new_classifier_weight'):
            model.module.new_classifier_weight.requires_grad = True
        if hasattr(model.module, 'new_classifier_bias'):
            model.module.new_classifier_bias.requires_grad = True
    ...
    def validate_for_CAM(self, loader, metrics, ret_samples_ids=None, logger=None, end_task=False, class_id=None): # this code is like a shit and will be cleaned
        ...
        if True:
            for i, (images, labels) in enumerate(loader):
                ...
                normalized_masks = torch.nn.functional.softmax(outputs, dim=1).cpu()
                class_mask = normalized_masks[0, :, :, :].argmax(axis=0).detach().cpu().numpy()
                class_mask_uint8 = 255 * np.uint8(class_mask == class_id)
                class_mask_float = np.float32(class_mask == class_id)
                ori_image = self.tensor_to_image(images, -1)
                both_images = np.hstack((ori_image, np.repeat(class_mask_uint8[:, :, None], 3, axis=-1)))
                img_and_mask = Image.fromarray(both_images)
                # img_and_mask.save("test.jpg")
                target_layers = [model.module.body.mod5]
                targets = [SemanticSegmentationTarget(class_id, class_mask_float)]
                from pytorch_grad_cam import GradCAM
                with GradCAM(model=model,
                    ...
                    # 'pip install git+https://github.com/jacobgil/pytorch-grad-cam.git' rather than 'pip install'
                    ...
                    img_cam.save("CAM_viz/" + str(i) + ".jpg")
